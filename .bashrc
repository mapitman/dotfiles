...
then
    ...
    then
        alias update="if type snap > /dev/null 2>&1; then sudo snap refresh; fi; if type flatpak > /dev/null 2>&1; then sudo flatpak update; fi; sudo apt-get update && sudo apt-get dist-upgrade -y && sudo apt-get autoremove -y"   
